    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2 justify-end">
        <div className="relative flex items-center gap-1">
          <input type="text" placeholder="Cari nama / WA..." value={searchInput} onChange={e => setSearchInput(e.target.value)} onKeyDown={handleKeyDown} className="w-36 pl-2.5 pr-2 py-1.5 bg-zinc-950 border border-zinc-800 rounded-lg text-[11px] text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-emerald-500/50 transition" />
          <button onClick={doSearch} className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[10px] font-bold transition cursor-pointer">Cari</button>
          {searchQuery && <span className="text-[9px] text-emerald-400 font-bold whitespace-nowrap">Filter: "{searchQuery}"</span>}
